...
- Issue #113 closes the forward reference to "force-push as a recovery primitive at push time" from the alternatives section above. The realisation is the workspace `push_branch` helper's invocation shape: `git push --force-with-lease=<branch> -u origin <branch>`. The lease's expected value is the local remote-tracking ref, which git maintains automatically across this pipeline's earlier pushes within the same workspace. Within the bellows-owned `agent/*` namespace the lease matches across history-rewriting operations (amend, split, rebase), so the per-finding review-fix agent's rewrites land at push time without bellows fetch-and-rebase plumbing. An external writer changing origin between bellows pushes (out-of-norm for the bellows-owned namespace) fails the lease and surfaces git's stderr through the existing git-failure error path; the deletion-time pre-claim sweep above and the push-time lease are independent slices of the same `agent/*`-ownership design.
