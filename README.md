...
progresses; override the strings via `[runtime_labels]` in
...
- `agent-done` — the run finished successfully; PR opened, `cargo
  test` was green. Informational agent notes surface as an advisory
  `## Agent notes` PR comment but no longer hold the merge (ADR-0011).
